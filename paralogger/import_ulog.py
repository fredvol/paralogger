...
def add_rk_angle(df_i):
    ...
    if mean_r_BGeo[2]<min_mean_r_BGeo_z: #mean(r.k)=mean_r_BGeo[2] has to be near but cannot be 1.0
        if mean_r_BGeo[2]<-min_mean_r_BGeo_z:
            ...
            logger.info("vertical_vector r might not have been guessed correctly, mean_r_BGeo = " + str(mean_r_BGeo))
...
def ulog_to_df(file_path):
    ...
    #Calculate absolute angle to vertical
    add_rk_angle(df_G)
